...
The project has three binaries. You'll typically want all three running at the same time.
...
**Worker** — processes RabbitMQ events (saves articles, downloads images):
```bash
make worker
```

**Feed updater** — one-shot command that re-fetches all registered feeds:
```bash
make update
```
